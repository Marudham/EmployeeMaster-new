...
export default function EmpRegister() {
 ...
 const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        const response = await axios.post("http://localhost:8080/ems/controller/empRegister", values);
  ...
  return (
    <div style={{backgroundColor:'#fff', border: '1px solid'}} className='eu-container'>
      <h2 className='eu-header'>Employee Registeration</h2>
      <form className='eu-form' onSubmit={formik.handleSubmit}>
        ...
        <div className='eu-form-group'>
          ...
          />
          {formik.touched.password && formik.errors.password && <div className='eu-error'>{formik.errors.password}</div>}
        </div>
        ...
        <div className='eu-form-group'>
          ...
          {formik.touched.address && formik.errors.address && (
            ...
            <div className='eu-form-group'>
            { loading ? (
                <button style={{ width:'100%', backgroundColor:'#3480db', cursor:'not-allowed'}} className='au-button' type='submit'>Registering...</button>
            ) : (
                <button style={{ width:'100%', backgroundColor:'#3498db'}} className='au-button' type='submit'>Register</button>
            )}
            </div>
            <p className='login-link-text'>
                Already Registered? <Link className='login-link' to={"/emplogin"}>Login</Link>
            </p>
        </form>
    </div>
  )
